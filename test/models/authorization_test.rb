...
class AuthorizationTest < ActiveSupport::TestCase
  ...
  test 'authorization_uri_should_returen_uri_string' do
    ...
    assert_equal 'id_token', query['response_type'].first
  ...
  test 'validate_when_id_token_verify_ok_should_return_true' do
    ...
    id_token.expects(:raw_attributes).returns({'userinfo' => {'sub' => '123'}})
    OpenIDConnect::ResponseObject::IdToken.expects(:decode).returns(id_token)
  ...
  test 'validate_when_invalid_state_should_return_false' do
    authorization = Authorization.new(@params)
    state = 'state_123'
    nonce = 'nonce_xyz'
